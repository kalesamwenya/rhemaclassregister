...
export default function AdminConfigScreen() {
  ...
  const { sysConfig, saveAllConfig, adminPassword, persistAdminPassword } =
    useAppContext();

  const [cfg, setCfg] = useState(sysConfig);
  const [passRotation, setPassRotation] = useState({
    current: "",
    next: "",
    confirm: "",
  });
  ...
  const renderCalendar = () => {
    ...
    const currentVal =
      pickerModal.termId && pickerModal.field
        ? cfg.terms[pickerModal.termId][pickerModal.field]
        : "";
      ...
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <Animated.View entering={FadeInDown.delay(100)} style={styles.section}>
          ...
          <View style={styles.card}>
            <View style={styles.inputGroup}>
              ...
              <TextInput
                style={styles.input}
                value={cfg.textCustoms["lbl-gate-title"]}
                onChangeText={(v) => updateText("lbl-gate-title", v)}
              />
            </View>
            <View style={styles.inputGroup}>
              ...
              <TextInput
                style={styles.input}
                value={cfg.textCustoms["lbl-gate-subtitle"]}
                onChangeText={(v) => updateText("lbl-gate-subtitle", v)}
              />
            </View>
            <View style={styles.inputGrid}>
              <View style={[styles.inputGroup, { flex: 1 }]}>
                ...
                <TextInput
                  style={styles.input}
                  value={cfg.textCustoms["lbl-terminal-title"]}
                  onChangeText={(v) => updateText("lbl-terminal-title", v)}
                />
              </View>
            </View>
              ...
              {Object.entries(cfg.cohorts).map(([id, label]) => (
                <View key={id} style={[styles.inputGroup, { width: "47%" }]}>
                  ...
                  <TextInput
                    ...
                    onChangeText={(v) => updateCohort(id, v)}
                  />
                </View>
              ))}
            </View>
        ...
        <Animated.View entering={FadeInDown.delay(250)} style={styles.section}>
          <View style={styles.sectionLabelRow}>
            ...
            <Text style={[styles.sectionLabel, { color: "#059669" }]}>
              ACADEMIC TERMS (CALENDAR)
            </Text>
          </View>
          <View style={styles.card}>
            <Text style={styles.helperText}>
              Define dates to automate student payment status checks.
            </Text>
            {Object.entries(cfg.terms || {}).map(
              ([id, dates]: [string, any]) => (
          ...
          <View style={styles.card}>
            <View style={styles.inputGroup}>
              ...
              <TextInput
                style={styles.input}
                secureTextEntry
                value={passRotation.current}
                onChangeText={(t) =>
                  setPassRotation({ ...passRotation, current: t })
                }
                placeholder="••••••••"
              />
            </View>
            <View style={styles.inputGrid}>
              <View style={[styles.inputGroup, { flex: 1 }]}>
                ...
                <TextInput
                  style={styles.input}
                  secureTextEntry
                  value={passRotation.next}
                  onChangeText={(t) =>
                    setPassRotation({ ...passRotation, next: t })
                  }
                />
              </View>
              <View style={[styles.inputGroup, { flex: 1 }]}>
                ...
                <TextInput
                  style={styles.input}
                  secureTextEntry
                  value={passRotation.confirm}
                  onChangeText={(t) =>
                    setPassRotation({ ...passRotation, confirm: t })
                  }
                />
              </View>
            </View>
