...
export default function AdminTimetableScreen() {
  ...
  const { schedule, students, sysConfig, saveAllSchedule, removeSchedule, courseEnrollments, saveAllEnrollments } = useAppContext();
  ...
  const [deleteConfirmation, setDeleteConfirmation] = useState<{ visible: boolean; id: string; name: string }>({
    visible: false,
    id: '',
    name: ''
  });
  ...
  const handlePickerSelect = (val: string) => {
    if (pickerModal.field === 'startDate' || pickerModal.field === 'endDate' || pickerModal.field === 'start' || pickerModal.field === 'end') {
      if (managingSlotId) {
  ...
  const renderCalendar = () => {
    ...
    const currentVal = (managingSlotId ? editFormData[pickerModal.field] : newSlot[pickerModal.field]) || '';
  ...
  const openManageModal = (slot: any) => {
    ...
    setEditFormData({ ...slot, cohort: String(slot.cohort) });
    setManageTab('roster');
  };
  ...
  const currentRoster = useMemo(() => {
    ...
    return (courseEnrollments[managingSlotId] || []).map(id => ({ id, ...(students[id] || { name: 'Unknown Student' }) }));
  ...
  const removeStudentsFromRoster = (ids: string[]) => {
    if (!managingSlotId) return;
    const next = { ...courseEnrollments, [managingSlotId]: (courseEnrollments[managingSlotId] || []).filter(id => !ids.includes(id)) };
  ...
  const bulkAddCohortToRoster = (cohortId: string) => {
    ...
    Object.keys(students).forEach(id => {
      if (parseInt(students[id].cohort) === parseInt(cohortId) && !ids.includes(id)) {
        ids.push(id);
      }
    });
      ...
      <View style={styles.cardTop}>
        <View style={styles.courseHeader}>
          ...
          <View style={styles.cohortPill}>
            <Text style={styles.cohortPillText}>{sysConfig.cohorts[item.cohort]}</Text>
          </View>
        </View>
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.searchArea}>
        <View style={styles.searchBar}>
          ...
          <TextInput style={styles.searchInput} placeholder="Search tracks..." value={timetableSearch} onChangeText={setTimetableSearch} />
        </View>
      </View>
      ...
      <FlatList
        ...
        ListEmptyComponent={<Text style={styles.emptyText}>No schedules mapped. Create one above.</Text>}
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <View style={styles.modalHeader}>
              ...
              <TouchableOpacity onPress={() => setCreateModalVisible(false)} style={styles.closeBtn}><X size={20} color={Colors.text} /></TouchableOpacity>
            </View>
            <ScrollView style={styles.modalBody}>
              <View style={styles.modalForm}>
                <Text style={styles.fieldLabel}>COURSE MODULE NAME</Text>
                <TextInput style={styles.modalInput} value={newSlot.course} onChangeText={t => setNewSlot({...newSlot, course: t})} placeholder="e.g. BTH 101" />

                <Text style={styles.fieldLabel}>TARGET COHORT</Text>
                <View style={styles.cohortGrid}>
                  {Object.entries(sysConfig.cohorts).map(([id, label]) => (
                    <TouchableOpacity key={id} style={[styles.cohortOption, newSlot.cohort === id && styles.cohortOptionActive]} onPress={() => setNewSlot({...newSlot, cohort: id})}>
                      <Text style={[styles.cohortOptionText, newSlot.cohort === id && styles.cohortOptionTextActive]}>{label as string}</Text>
                    </TouchableOpacity>
                  ))}
                </View>

                <TouchableOpacity style={styles.toggleRow} onPress={() => setNewSlot({...newSlot, autoEnroll: !newSlot.autoEnroll})}>
                  <View style={[styles.checkbox, newSlot.autoEnroll && styles.checkboxActive]}>
                    {newSlot.autoEnroll && <X size={12} color="#fff" />}
                  </View>
                  <Text style={styles.toggleText}>Automatically bulk-enroll cohort students</Text>
                </TouchableOpacity>

                <View style={styles.inputGrid}>
                  <View style={{ flex: 1 }}>
                    <Text style={styles.fieldLabel}>START DATE</Text>
                    <TouchableOpacity style={styles.modalInputTouch} onPress={() => openPicker('date', 'startDate')}>
                      <Calendar size={18} color={Colors.primary} />
                      <Text style={styles.modalInputTouchText}>{newSlot.startDate}</Text>
                    </TouchableOpacity>
                  </View>
                  <View style={{ flex: 1 }}>
                    <Text style={styles.fieldLabel}>END DATE</Text>
                    <TouchableOpacity style={styles.modalInputTouch} onPress={() => openPicker('date', 'endDate')}>
                      <Calendar size={18} color={Colors.primary} />
                      <Text style={styles.modalInputTouchText}>{newSlot.endDate}</Text>
                    </TouchableOpacity>
                  </View>
                </View>
                ...
                <View style={styles.inputGrid}>
                  <View style={{ flex: 1 }}>
                    ...
                    <TouchableOpacity style={styles.modalInputTouch} onPress={() => openPicker('time', 'start')}>
                      <Clock size={18} color={Colors.primary} />
                      <Text style={styles.modalInputTouchText}>{newSlot.start}</Text>
                    </TouchableOpacity>
                  </View>
                  <View style={{ flex: 1 }}>
                    <Text style={styles.fieldLabel}>END TIME</Text>
                    <TouchableOpacity style={styles.modalInputTouch} onPress={() => openPicker('time', 'end')}>
                      <Clock size={18} color={Colors.primary} />
                      <Text style={styles.modalInputTouchText}>{newSlot.end}</Text>
                    </TouchableOpacity>
                  </View>
                </View>
                ...
                <View style={styles.labelRow}>
                  ...
                  <TouchableOpacity onPress={() => setNewSlot({...newSlot, days: newSlot.days.length === 7 ? [] : [0,1,2,3,4,5,6]})}>
                    <Text style={styles.labelActionText}>{newSlot.days.length === 7 ? 'Clear' : 'All'}</Text>
                  </TouchableOpacity>
                </View>
                <View style={styles.daysGrid}>
                  {DAYS_FULL.map((day, idx) => (
                    <TouchableOpacity key={day} style={[styles.dayToggle, newSlot.days.includes(idx) && styles.dayToggleActive]} onPress={() => {
                      const days = newSlot.days.includes(idx) ? newSlot.days.filter(d => d !== idx) : [...newSlot.days, idx];
                      setNewSlot({...newSlot, days});
                    }}>
                      <Text style={[styles.dayToggleText, newSlot.days.includes(idx) && styles.dayToggleTextActive]}>{day.substring(0, 3)}</Text>
                    </TouchableOpacity>
                  ))}
                </View>

                <TouchableOpacity style={styles.saveBtn} onPress={handleCreateCourse}>
                  <Text style={styles.saveBtnText}>Inject Track into Timetable</Text>
                </TouchableOpacity>
              </View>
            </ScrollView>
          </View>
          ...
          <View style={styles.modalContentFull}>
            <View style={styles.modalHeader}>
              <View>
                ...
                <Text style={styles.modalSubtitle}>{sysConfig.cohorts[editFormData?.cohort]}</Text>
              </View>
              <TouchableOpacity onPress={() => setManagingSlotId(null)} style={styles.closeBtn}><X size={20} color={Colors.text} /></TouchableOpacity>
            </View>

            <View style={styles.tabBarInner}>
              <TouchableOpacity style={[styles.innerTab, manageTab === 'roster' && styles.innerTabActive]} onPress={() => setManageTab('roster')}>
                 <Users size={18} color={manageTab === 'roster' ? Colors.primary : '#94a3b8'} />
                 <Text style={[styles.innerTabText, manageTab === 'roster' && styles.innerTabTextActive]}>Roster</Text>
              </TouchableOpacity>
              <TouchableOpacity style={[styles.innerTab, manageTab === 'schedule' && styles.innerTabActive]} onPress={() => setManageTab('schedule')}>
                 <Settings size={18} color={manageTab === 'schedule' ? Colors.primary : '#94a3b8'} />
                 <Text style={[styles.innerTabText, manageTab === 'schedule' && styles.innerTabTextActive]}>Settings</Text>
              </TouchableOpacity>
            </View>
              ...
              <View style={{ flex: 1 }}>
                <View style={styles.rosterWizard}>
                  ...
                  <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginBottom: 15 }}>
                    {Object.entries(sysConfig.cohorts).map(([id, label]) => (
                      <TouchableOpacity key={id} style={styles.bulkPill} onPress={() => bulkAddCohortToRoster(id)}>
                        <Text style={styles.bulkPillText}>+ {label as string}</Text>
                      </TouchableOpacity>
                    ))}
                  </ScrollView>
                  <View style={styles.rosterActions}>
                    <View style={styles.searchBarMini}>
                      ...
                      <TextInput style={styles.searchInputMini} placeholder="Filter roster..." value={rosterSearch} onChangeText={setRosterSearch} />
                    </View>
                    <TouchableOpacity style={styles.addSingleBtn} onPress={() => setShowSinglePicker(true)}><UserPlus size={20} color="#fff" /></TouchableOpacity>
                  </View>
                </View>
                ...
                <FlatList
                  ...
                  renderItem={({item}) => (
                    <View style={styles.rosterRow}>
                      ...
                      <TouchableOpacity onPress={() => removeStudentsFromRoster([item.id])} style={styles.removeBtn}><X size={16} color={Colors.danger} /></TouchableOpacity>
                    </View>
                  )}
                  contentContainerStyle={{ paddingBottom: 40 }}
                  ListEmptyComponent={<Text style={styles.emptyText}>Roster is empty.</Text>}
                />
              </View>
            ) : (
              <ScrollView style={styles.modalBody}>
                <View style={styles.modalForm}>
                  ...
                  <TextInput style={styles.modalInput} value={editFormData.course} onChangeText={t => setEditFormData({...editFormData, course: t})} />
                  ...
                  <View style={styles.inputGrid}>
                    <View style={{ flex: 1 }}>
                      ...
                      <TouchableOpacity style={styles.modalInputTouch} onPress={() => openPicker('date', 'startDate')}>
                        <Calendar size={18} color={Colors.primary} />
                        <Text style={styles.modalInputTouchText}>{editFormData.startDate}</Text>
                      </TouchableOpacity>
                    </View>
                    <View style={{ flex: 1 }}>
                      <Text style={styles.fieldLabel}>END DATE</Text>
                      <TouchableOpacity style={styles.modalInputTouch} onPress={() => openPicker('date', 'endDate')}>
                        <Calendar size={18} color={Colors.primary} />
                        <Text style={styles.modalInputTouchText}>{editFormData.endDate}</Text>
                      </TouchableOpacity>
                    </View>
                  </View>
                  ...
                  <View style={styles.inputGrid}>
                    <View style={{ flex: 1 }}>
                      ...
                      <TouchableOpacity style={styles.modalInputTouch} onPress={() => openPicker('time', 'start')}>
                        <Clock size={18} color={Colors.primary} />
                        <Text style={styles.modalInputTouchText}>{editFormData.start}</Text>
                      </TouchableOpacity>
                    </View>
                    <View style={{ flex: 1 }}>
                      <Text style={styles.fieldLabel}>END TIME</Text>
                      <TouchableOpacity style={styles.modalInputTouch} onPress={() => openPicker('time', 'end')}>
                        <Clock size={18} color={Colors.primary} />
                        <Text style={styles.modalInputTouchText}>{editFormData.end}</Text>
                      </TouchableOpacity>
                    </View>
                  </View>

                  <TouchableOpacity style={styles.saveBtn} onPress={handleUpdateSchedule}><Text style={styles.saveBtnText}>Save Parameters</Text></TouchableOpacity>
                  <TouchableOpacity
                    style={styles.deleteLink}
                    onPress={() => deleteScheduleSlot(managingSlotId!, editFormData.course)}
                    activeOpacity={0.7}
                  >
                    <Trash2 size={16} color={Colors.danger} style={{ marginRight: 8 }} />
                    <Text style={styles.deleteLinkText}>Permanently Delete Course Track</Text>
                  </TouchableOpacity>
                </View>
              </ScrollView>
            )}
        ...
        <View style={styles.alertOverlay}>
          <Animated.View entering={FadeInDown.springify()} style={styles.alertBox}>
            ...
            <Text style={styles.alertMessage}>
              Are you sure you want to permanently remove <Text style={styles.alertStrong}>{deleteConfirmation.name}</Text>? This action cannot be undone and will wipe all associated student rosters.
            </Text>

            <View style={styles.alertActions}>
              <TouchableOpacity
                style={styles.alertCancelBtn}
                onPress={() => setDeleteConfirmation({ ...deleteConfirmation, visible: false })}
              >
...
const styles = StyleSheet.create({
  ...
  slotCard: { backgroundColor: '#fff', borderRadius: 24, padding: 20, borderWidth: 1, borderColor: '#f1f5f9', elevation: 4, shadowColor: '#000', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.05, shadowRadius: 10 },
  cardTop: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12 },
  courseHeader: { flex: 1, gap: 6 },
  courseNameText: { fontSize: 18, fontWeight: '900', color: '#1e293b', letterSpacing: -0.2 },
  cohortPill: { alignSelf: 'flex-start', backgroundColor: '#0f172a', paddingHorizontal: 8, paddingVertical: 4, borderRadius: 8 },
  cohortPillText: { color: '#fff', fontSize: 10, fontWeight: '900', textTransform: 'uppercase' },
  manageBtn: { width: 36, height: 36, borderRadius: 10, backgroundColor: Colors.blue50, alignItems: 'center', justifyContent: 'center' },
  ...
  dayBadge: { paddingHorizontal: 10, paddingVertical: 6, borderRadius: 10, backgroundColor: '#f8fafc', borderWidth: 1, borderColor: '#f1f5f9' },
  ...
  modalContent: { backgroundColor: '#fff', borderTopLeftRadius: 32, borderTopRightRadius: 32, paddingBottom: 40, maxHeight: '85%' },
  modalContentFull: { backgroundColor: '#fff', borderTopLeftRadius: 32, borderTopRightRadius: 32, height: '92%' },
  ...
  modalSubtitle: { fontSize: 12, fontWeight: '800', color: Colors.primary, marginTop: 2, textTransform: 'uppercase' },
  ...
  modalInputTouch: { backgroundColor: '#f8fafc', borderWidth: 1, borderColor: '#e2e8f0', borderRadius: 16, padding: 15, flexDirection: 'row', alignItems: 'center', gap: 10, height: 55 },
  ...
  toggleRow: { flexDirection: 'row', alignItems: 'center', gap: 12, backgroundColor: Colors.blue50, padding: 15, borderRadius: 16 },
  ...
  toggleText: { fontSize: 13, fontWeight: '700', color: Colors.primary, flex: 1 },

  inputGrid: { flexDirection: 'row', gap: 12 },
  labelRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-end' },
  labelActionText: { fontSize: 12, fontWeight: '900', color: Colors.primary, textTransform: 'uppercase' },

  daysGrid: { flexDirection: 'row', flexWrap: 'wrap', gap: 8 },
  dayToggle: { width: '22%', height: 40, borderRadius: 10, backgroundColor: '#f8fafc', alignItems: 'center', justifyContent: 'center', borderWidth: 1, borderColor: '#e2e8f0' },
  ...
  tabBarInner: { flexDirection: 'row', borderBottomWidth: 1, borderBottomColor: '#f1f5f9' },
  innerTab: { flex: 1, height: 50, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8 },
  ...
  rosterWizard: { padding: 20, backgroundColor: '#fcfdfe', borderBottomWidth: 1, borderBottomColor: '#f1f5f9' },
  wizardLabel: { fontSize: 10, fontWeight: '900', color: '#94a3b8', marginBottom: 10, textTransform: 'uppercase' },
  bulkPill: { paddingHorizontal: 12, paddingVertical: 6, borderRadius: 12, backgroundColor: '#fff', borderWidth: 1, borderColor: '#e2e8f0', marginRight: 8 },
  bulkPillText: { fontSize: 11, fontWeight: '800', color: Colors.primary },
  rosterActions: { flexDirection: 'row', gap: 10 },
  searchBarMini: { flex: 1, flexDirection: 'row', alignItems: 'center', backgroundColor: '#f1f5f9', borderRadius: 12, paddingHorizontal: 12, height: 45 },
  searchInputMini: { flex: 1, marginLeft: 8, fontSize: 14, fontWeight: '600' },
  addSingleBtn: { width: 45, height: 45, borderRadius: 12, backgroundColor: Colors.primary, alignItems: 'center', justifyContent: 'center' },

  rosterRow: { flexDirection: 'row', alignItems: 'center', padding: 16, borderBottomWidth: 1, borderBottomColor: '#f1f5f9' },
  ...
  deleteLink: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', marginTop: 10, padding: 15 },
  deleteLinkText: { color: Colors.danger, fontWeight: '800', fontSize: 13 },

  pickerModalContent: { backgroundColor: '#fff', borderTopLeftRadius: 32, borderTopRightRadius: 32, height: '50%' },
