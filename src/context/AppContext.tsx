...
import { Alert, Platform, Share, useColorScheme } from "react-native";
...
import {
  deleteSchedule as deleteScheduleFromApi,
  getScheduleData,
  syncScheduleData,
...
interface AppContextType {
  ...
  saveAllSchedule: (
    data: any[],
    enrollmentsOverride?: Record<string, string[]>,
  ) => Promise<void>;
  ...
  getPaymentSummary: (value: string | undefined) => {
    status: string;
    label: string;
  };
...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const [sysConfig, setSysConfig] = useState({
    ...
    textCustoms: {
      ...
      "lbl-gate-subtitle":
        "Secure checkpoint management console & kiosk logger",
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const keys = [
        ...
        "rhema_enrollments",
        "rhema_sys_config",
        "rhema_attendance_records",
        "rhema_payment_status",
        "rhema_payment_alerts",
      ];
  ...
  const loadScheduleFromApi = async () => {
    try {
      ...
      if (res?.success) {
        ...
        await AsyncStorage.setItem(
          "rhema_schedule",
          JSON.stringify(res.data.schedules),
        );
        await AsyncStorage.setItem(
          "rhema_enrollments",
          JSON.stringify(res.data.enrollments),
        );
      }
    } catch {}
  };
        ...
        if (isBackground) {
          setPaymentAlerts((prev) => {
            const prevUnreadIds = new Set(
              prev.filter((a) => !a.is_read).map((a) => String(a.id)),
            );
            const newUnreads = newData.filter(
  ...
  const getPaymentSummary = (val: string | undefined) => {
    ...
    if (n === "balance")
      return { status: "partial", label: "Partial terms paid" };
    return { status: "pending", label: "No payment recorded" };
  };

  const hasFullTermPayment = (id: string) =>
    getPaymentSummary(paymentStatus[id]).status === "all-paid";

  const getCurrentTerm = () => {
    const now = new Date();
    const isoDate = now.toISOString().split("T")[0];
    const term = Object.entries(sysConfig.terms || {}).find(
      ([id, dates]: [string, any]) => {
        return (
          dates.start &&
          dates.end &&
  ...
  const registerStudent = async (student: any) => {
    ...
    const nextStudents = {
      ...students,
      [idStr]: { ...student, payment_status: pStatus },
    };
  ...
  const removeStudent = async (id: string) => {
    ...
    Object.keys(nextEnrollments).forEach((slotId) => {
      if (Array.isArray(nextEnrollments[slotId])) {
        nextEnrollments[slotId] = nextEnrollments[slotId].filter(
          (sid) => sid !== idStr,
        );
      }
    });
    ...
    try {
      ...
      console.error(
        `[SYNC] Network/Server error deleting student ${idStr}:`,
        e.message,
      );
    }
  };
  ...
  const saveAllSchedule = async (
    ...
    try {
      const res = await syncScheduleData({
        schedules: data,
      ...
      if (!res?.success) {
        console.warn(
          "Sync Warning",
          res?.message || "Failed to sync schedule with server",
        );
      }
    } catch (err) {
  ...
  const saveAllEnrollments = async (
    data: Record<string, string[]>,
    ...
    await AsyncStorage.setItem(
      "rhema_schedule",
      JSON.stringify(scheduleOverride),
    );
    ...
    try {
      ...
      if (!res?.success) {
        console.warn(
          "Sync Warning",
          res?.message || "Failed to sync enrollments with server",
        );
      }
    } catch (err) {
  ...
  const markAlertAsRead = async (id: number | string) => {
    const next = paymentAlerts.map((a) =>
      String(a.id) === String(id) ? { ...a, is_read: 1 } : a,
    );
  ...
  const saveAllConfig = async (data: any) => {
    ...
    try {
      ...
      if (!res?.success) {
        console.warn(
          "Remote Save Warning",
          res?.message || "Failed to sync config with server",
        );
      }
    } catch (err: any) {
  ...
  const downloadBackup = async () => {
    ...
    try {
      ...
      Alert.alert("Error", "Could not create backup share");
    }
  };
  ...
  return (
    <AppContext.Provider
      value={{
        ...
        getYearGroupFromCohort,
        getYearLabelFromCohort,
        isYearCompatible,
        getPaymentSummary,
        hasFullTermPayment,
        isPaymentCurrent,
